...
//public class FestivoServicio {
    ...
    @Autowired
    public FestivoServicio(IFestivoRepositorio iFestivoRepositorio){
		this.iFestivoRepositorio = iFestivoRepositorio;
	}
    public List<Festivo> listar() {
        return this.iFestivoRepositorio.findAll();
    }
    ...
    private Date siguienteLunes(Date fecha) {
        ...
        if (c.get(Calendar.DAY_OF_WEEK) > Calendar.MONDAY)
            fecha = agregarDias(fecha, 7 - c.get(Calendar.DAY_OF_WEEK));
    ...
    public List<Date> obtenerFestivos(int año) {
        List<Festivo> festivos = this.iFestivoRepositorio.findAll();
